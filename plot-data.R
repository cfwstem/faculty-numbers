...
stemfacultyplot
ggsave(filename = "plots/stemfacultyplot-align-right-70angle.svg", plot = stemfacultyplot, width = 8, height = 6, units = "in")
...
facultybypositionplot <- ggplot(data = facultybyposition, aes(x = Gender, y = freq, group = Designation, fill = Gender, colour = Gender))+
  ...
  #scale_fill_brewer(palette = "Set2") +
  geom_bar(colour = "#58595b", stat = "identity") +
  facet_grid(Designation~Position)+
  xlab("Gender") +
  ylab("Faculty Members") +
  ggtitle("STEM Faculty by Gender at Tennessee-Knoxville") +
  theme(
...
ggsave(filename = "plots/facultybypositionplot.svg", plot = facultybypositionplot, width = 9.6, height = 6, units = "in")
